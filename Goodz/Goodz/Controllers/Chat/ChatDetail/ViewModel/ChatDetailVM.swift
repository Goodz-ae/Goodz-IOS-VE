...
class ChatDetailVM {
    ...
    var chatDetails : [ChatDetailModel] = []
    var dataChatMainResponse : ChatMainResponse?
            ...
            if let chat = data {
                if page == 1, let msg = chat.first?.messages {
                    ...
                    if let pList = detailsChat?.itemToConfirm {
                        ...
                        self.arrChat!.append(MessageModel(productDetails:pList  , is_seller :mainres.isSeller  , delivery_method:mainres.deliveryMethod , buyerName :chat.first?.name ?? ""   , slotOptions : detailsChat?.slotOptions , slot_message_id : pList.slot_message_id ))
                        self.arrChat = self.arrChat?.reversed()
                    }
