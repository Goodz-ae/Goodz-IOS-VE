...
@objc open class ExpandableLabel: UILabel {
    ...
    open func setMoreLinkWith(moreLink: String, attributes: [NSAttributedString.Key: AnyObject], position: NSTextAlignment?) {
        var alignedattributes = attributes
        if let pos = position {
            // expandedLinkPosition = pos
            let titleParagraphStyle = NSMutableParagraphStyle()
        ...
        collapsedAttributedLink = NSMutableAttributedString(string: moreLink,
                                                           attributes: alignedattributes)
    }
}
...
private extension NSAttributedString {
    ...
    func copyWithAddedFontAttribute(_ font: UIFont) -> NSAttributedString {
        if !hasFontAttribute() {
           ...
           // copy.addAttribute(.foregroundColor, value: UIColor.red, range: NSRange(location: 0, length: copy.length))
            return copy
        }
